            ...
            when (t) {
                SS::class.java ->
                    NODE_SS.writeLine(data).also {
                        println("ss节点: ${u.size}".also { nodeInfo.writeLine("- $it") })
                    }
                SSR::class.java ->
                    NODE_SSR.writeLine(data).also {
                        println("ssr节点: ${u.size}".also { nodeInfo.writeLine("- $it") })
                    }
                V2ray::class.java ->
                    NODE_V2.writeLine(data).also {
                        println("v2ray节点: ${u.size}".also { nodeInfo.writeLine("- $it") })
                    }
                Trojan::class.java ->
                    NODE_TR.writeLine(data).also {
                        println("trojan节点: ${u.size}".also { nodeInfo.writeLine("- $it") })
                    }
            }
        }
    }
